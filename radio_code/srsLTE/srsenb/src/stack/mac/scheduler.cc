...
{
  uint32_t cqi_to_use = cqi_value;

  // SCOPE: force downlink modulation if specified
  if (is_user(rnti)) {
    int ue_array_idx = get_ue_idx_from_rnti(rnti);
    uint32_t user_cqi = cqi_from_modulation(ue_resources[ue_array_idx].dl_modulation);
...
//    printf("DL: RNTI %" PRIu16 ", using CQI %" PRIu32 "\n", rnti, cqi_to_use);
  }

  return ue_db_access(rnti, [tti, enb_cc_idx, cqi_to_use](sched_ue& ue) { ue.set_dl_cqi(tti, enb_cc_idx, cqi_to_use); });
}
...
{
  uint32_t cqi_to_use = cqi;

  // SCOPE: force uplink modulation if specified
  if (is_user(rnti)) {
    int ue_array_idx = get_ue_idx_from_rnti(rnti);
    uint32_t user_cqi = cqi_from_modulation(ue_resources[ue_array_idx].ul_modulation);
...
//    printf("UL: RNTI %" PRIu16 ", using CQI %" PRIu32 "\n", rnti, cqi_to_use);
  }

  return ue_db_access(rnti, [&](sched_ue& ue) { ue.set_ul_cqi(tti, enb_cc_idx, cqi_to_use, ul_ch_code); });
}

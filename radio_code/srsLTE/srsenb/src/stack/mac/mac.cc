...
void mac::reset()
{
  ...
  // SCOPE: use macro to store the first valid user RNTI
  last_rnti = FIRST_VALID_USER_RNTI;
...
// SCOPE: pass extra argument transmission_mode
int mac::cqi_info(uint32_t tti, uint16_t rnti, uint32_t enb_cc_idx, uint32_t cqi_value, uint32_t transmission_mode)
{
  long int last_time_written = 0;
  long int timestamp_ms;
  int writing_period_ms = 5000;
  ...
  if (ue_db.count(rnti)) {
    ...
    // SCOPE: periodically write RNTI::SNR on file
    // get current timestamp
    timestamp_ms = get_time_milliseconds();

    // get last timestamp at which metric was written on file
    try {
        last_time_written = rnti_metric_timestamp.at(rnti);
    }
    catch (std::out_of_range) {
        last_time_written = 0;
    }

    if (timestamp_ms - last_time_written > writing_period_ms) {
        // update timestamp value in map
        rnti_metric_timestamp[rnti] = timestamp_ms;

        // get sinr approximation and save it in user structure
        double dl_sinr = sinr_from_cqi(transmission_mode, cqi_value);
        int ue_array_idx = get_ue_idx_from_rnti(rnti);
        ue_resources[ue_array_idx].dl_sinr = dl_sinr;
    }

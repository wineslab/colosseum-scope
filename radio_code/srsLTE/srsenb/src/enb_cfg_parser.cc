...
int parse_cfg_files(all_args_t* args_, rrc_cfg_t* rrc_cfg_, phy_cfg_t* phy_cfg_)
{
  ...
  // SCOPE: read parameter to check if we are running on Colosseum
  // NOTE: moved here, previously in srsenb/src/phy/txrx.cc
  std::string global_config_dir_path = SCOPE_CONFIG_DIR;
  std::string gloabl_config_file_name = "scope_cfg.txt";
  colosseum_testbed = (int) read_config_parameter(global_config_dir_path, gloabl_config_file_name, "colosseum_testbed");

  // set to 0 if parameter not found in config file
  if (colosseum_testbed == -1)
      colosseum_testbed = 0;
...
int set_derived_args(all_args_t* args_, rrc_cfg_t* rrc_cfg_, phy_cfg_t* phy_cfg_, const srslte_cell_t& cell_cfg_)
{
  ...
  if (rrc_cfg_->cell_list.size() == 1) {
    ...
    // SCOPE: allow to specify uplink frequency in config file if running on Colosseum
    if (args_->rf.ul_freq > 0) {
      if (colosseum_testbed) {
        ...
        ERROR("Forcing UL freq not allowed\n");
      }
    }
  } else {

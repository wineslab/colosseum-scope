...
struct sched_ue_carrier {
  ...
  int                        get_required_prb_dl(uint32_t req_bytes, uint32_t nof_ctrl_symbols, bool sc_stat);
...
public:
  ...
  // SCOPE: function to get number of required PRBs in downlink
  ...
  rbg_range_t get_rbgs_from_prbs(uint32_t ue_cc_idx, int prb_num);
  ...
  // SCOPE: count for times user was scheduled for DL transmission
  int times_scheduled;

  // SCOPE: timestamp of last time allocated PRB number was logged on file
  long int last_time_prb_logged_ms;

  // SCOPE: function to get cell PRBs
  uint32_t get_serving_cell_prbs();

  // SCOPE: to keep track of the user slice ownership
  int slice_number;

  // SCOPE: save user slicing mask
  int has_slicing_mask;
  rbgmask_t slicing_mask;

  // SCOPE: to save user IMSI
  long long unsigned int imsi;

  // SCOPE: modulation functions
  int get_ul_fixed_mcs(uint32_t cc_idx);
  int get_dl_fixed_mcs(uint32_t cc_idx);
  void set_fixed_mcs_dl(int mcs_dl, uint32_t cc_idx);
